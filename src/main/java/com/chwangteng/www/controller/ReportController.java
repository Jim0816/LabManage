...
import com.chwangteng.www.Utils.MailUtil;
...
public class ReportController {
    ...
    public ModelAndView addReport(@RequestBody AddReportRequestParam addReportRequestParam, HttpSession session) throws Exception {
...
/*        String finaltitle = "";
        ...
        if(rows==1){
            ...
            // 发送邮件
            String emailTitle = "周报系统提醒 - " +student.getName() + "提交周报";
            String content = "<h3>本周总结</h3><br>" + report.getThisWeek() +
                    "<br><br><h3>遇到的困难</h3><br>" + report.getBugMeet()
                    + "<br><br><h3>下周计划</h3><br>" +report.getNextWeek();
            try {
                // 发送普通文本邮件
                mailUtil.sendMail(teacher.getMail(), emailTitle, content);
            } catch (Exception e) {
                e.printStackTrace();
            }
