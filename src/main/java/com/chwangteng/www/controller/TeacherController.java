...
import com.chwangteng.www.Utils.CronUtil;
...
/*                if(lab!=null){
    ...
    public ModelAndView updateDeadline(@RequestBody UpdateDeadlineParam updateDeadlineParam, HttpSession session)   {
        ...
        if(usertype==ConstVar._TEACHER_){
            ...
            String time = updateDeadlineParam.getDate();//需要转成四位数,如2200
            String cron = CronUtil.getCronStr(Integer.valueOf(day), time);
            teacher.setDeadline(cron);
            int rows = teacherMapper.updateByPrimaryKeySelective(teacher);
            if(rows==1){
                // 修改调度
                scheduledExecutorTask.setTimingDate(cron);
                Map<String,Object> map = new HashMap<String,Object>();
    ...
    //老师设置周报截止日期
    @RequestMapping("/getDeadline.action")
    public ModelAndView getDeadline(HttpSession session)   {
        int userid = Integer.parseInt(session.getAttribute(ConstVar._SESSION_USER_ID_).toString());
        int usertype = Integer.parseInt(session.getAttribute(ConstVar._SESSION_USER_TYPE_).toString());
        if(usertype==ConstVar._TEACHER_){
            Teacher teacher = teacherMapper.selectByPrimaryKey(userid);
            Map<String, String> map = new HashMap<>();
            String[] deadline = CronUtil.reverseCron(teacher.getDeadline());
            map.put("day", deadline[0]);
            map.put("date", deadline[1]);
            map.put(ConstVar._KEY_MESSAGE_, "获取截至期限");
            return new ModelAndView(new MappingJackson2JsonView(),map);
        }else{
            Map<String,Object> map = new HashMap<String,Object>();
            map.put(ConstVar._KEY_CODE_, ConstVar._ERROR_COMMON_);
